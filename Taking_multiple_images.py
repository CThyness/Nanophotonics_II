...
top_camera_ID = 'kajshdfkjsah' #Placeholder
front_camera_ID = 'lkafj;lkdsajfl;ka' #Placeholder
...
exposure_time = exposure_time_top_IR #Change to current light source
...
#Make all directories required for the full file path to exist
makedirs(folder_path)

with Vimba.get_instance() as vimba:
    with vimba.get_camera_by_id(camera_ID) as cam: #SHOULD work
        
        cam.get_feature_by_name('PixelFormat').set('Mono16')
        cam.get_feature_by_name('ExposureTime').set(exposure_time)
        
        i = 0
        for frame in cam.get_frame_generator(limit=N_pictures): #Takes 10 images and saves each as a unique .tiff file
            ...
            im.save(path.join(folder_path, f'{i}.tif')) #Again, SHOULD work
            i += 1
